...
class Auth{
	...
	public static function post($request){
		...
		if ($request == 'login') {
			...
			return ['status' => 'error', 'data' => 'malformed URL'];
	...
	private static function login(){
		...
		if (is_null($request)) {
			...
			$userPass = $authModel -> getPass($email);
			$userData = $authModel -> getUser($email);
			...
			if ($pass_verify == true) {
				...
				$sms_send_pin = $authModel -> send_pin($userData);

				//consultar calificación del usuario
				//$userRate = $authModel -> getRate($email,$dbPass);
	...
	private static function logout($params){
		...
		if (is_null($params)) {
			...
			if ($deleteToken) {
				...
				//array de respuesta
				$response = array(
					'user' => 'session logout successful'
				);
