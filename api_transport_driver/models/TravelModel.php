...
class TravelModel
{
	...
	function travel_details($params){
		...
		switch ($params['status']) {
			case 'F':
				$details = TravelModel::getFinalizedDetails($travel);//viaje finalizado correctamente
				break;

			case 'S':
				$details = TravelModel::getSolicitedDetails($travel);//viaje solicitado, falta confirmacion del conductor
				break;

			case 'P':
				$details = TravelModel::getPendingDetails($travel);//viaje pendiente, esperando al conductor
				break;

			case 'T':
				$details = TravelModel::getInTransitDetails($travel);//viaje pendiente, esperando al conductor
				break;
	...
	//aceptar el viaje > ir a recoger al pasajero
	function travel_accept($request){

		global $db;

		//obtener ID del usuario
		$driver = TravelModel::getDriverID($request);

		//verificar el viaje
		if(TravelModel::checkTravel($request)){

			//status del conductor - ocupado
			$status_driver = ['online' => 'O'];//occuped
			...
			//datos para actualizar el status del viaje
			$travel_accept = ['status' => 'P'];

			//actualizar status del viaje
			return $query_accept_travel = $db->table('transport_travel')
				->where('md5(id::varchar)',$request->travel)
				->where('md5(id_client_id::varchar)',$request->travel_passenger)
				->where('id_driver_id',$driver->driver)
				->update($travel_accept);
	...
	//iniciar el viaje > llevar al pasajero a su destino
	function travel_start($request){
		...
		$travel_id = TravelModel::getTravelID($request);

		//ID viaje request
		$travel = $request->travel;

		//acctualizar fecha/hora de inicio del viaje
		if(!is_null($request->travel_start_date)){
		
			$start_date = $request->travel_start_date;
		...
		//verificar el viaje
		if(TravelModel::getPendingDetails($travel)){

			//verificar si el codigo ingresado es correcto
			if(TravelModel::checkCodeTravel($request)){

				//actualizar punto de partida del viaje
				if(!is_null($request->travel_start_lat) && !is_null($request->travel_start_long)){
					
					//datos para actualizar el status del viaje
					$travel_update_data = [
						'status' 	 => 'T',
						'date_init'  => $start_date,
						'start_lat'  => $request->travel_start_lat,
						'start_long' => $request->travel_start_long
					];

				}else{

					//datos para actualizar el status del viaje
					$travel_update_data = ['status' => 'T', 'date_init' => $start_date];
				...
				$query = $db->table('transport_travel')
						...
						->where('md5(id_client_id::varchar)',$request->travel_passenger)
						->where('id_driver_id',$driver->driver)
						->update($travel_update_data);

				//datos para hacer el registro de la calificacion del viaje sin datos
				$rates_insert_data = [
					'travel_id' 	  => $travel_id->travel,
					'rate_client'  	  => '0',
					'rate_driver'  	  => '0',
					'comments_client' => '',
					'comments_driver' => ''];
				...
				return $db->table('transport_rates')->insert($rates_insert_data);

			}else{

				throw new ExceptionApi('error', 'travel code incorrect', 400);
	...
	//finalizar el viaje > el pasajero ha llegado a su destino
	function travel_finalize($params){
		...
		//datos del viaje
		$get_travel = TravelModel::getInTransitDetails($travel);

		//obtener ID del conductor
		$driver = TravelModel::getDriverID_EndTravel($params);
        ...
        if($end_distance > $travel_distance || $end_distance < $travel_distance){
            ...
            TravelModel::travel_cost_update($params,$travel_new_distance,$travel_new_duration,$travel_new_subtotal);
	...
	*/
	...
	private function travel_cost_update($params,$travel_new_distance,$travel_new_duration,$travel_new_subtotal){
		...
		$taxes = TravelModel::getTaxes();

		//obtener el ID del recibo
		$receipt_id = TravelModel::getReceipt($params);
