...
class RegionModel
{
	...
	function region_coverage($request){
		...
		//registrar el conductor y la region donde tiene cobertura
		RegionModel::driver_region_insert($driver->driver,$coverage_region[0]['reg_cov_id']);

		//retornar datos de la region
	...
	*/
	...
	//registrar la region del conductor
	private function driver_region_insert($driver,$region){

		global $db;		

		//borrar primero el registro para depurar
		$db->table('transport_driver_region')->where('id_driver',$driver)->delete();

		//registro del conductor y region
		$driver_region_data = ['id_driver' => $driver, 'id_region' => $region];

		$db->table('transport_driver_region')->insert($driver_region_data);
